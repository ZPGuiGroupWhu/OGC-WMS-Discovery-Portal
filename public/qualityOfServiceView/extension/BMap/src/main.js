    ...
    BMapExt.prototype._init = function (obj, BMap, ec, mapOption) {
        ...
        self.setOption = function (option, notMerge) {
            ...
            for (let i = 0;i<series.length;i++) {
                var geoCoord = series[i].geoCoord;
                if (geoCoord) {
                    for (let k of Object.keys(geoCoord)) {
            ...
            // 添加x、y
            for (let i = 0;i<series.length;i++) {
                var markPoint = series[i].markPoint || {};
                var markLine = series[i].markLine || {};
         ...
         */
        self._AddPos = function (tobj) {
            var coord = this._geoCoord[tobj.name]
            var pos = this.geoCoord2Pixel(coord);
            tobj.x = pos[0] - self._mapOffset[0];
            tobj.y = pos[1] - self._mapOffset[1];
        };
        ...
        function _moveHandler(type) {
            return function () {
                ...
                self._mapOffset = [
                    - parseInt(offsetEle.style.left,0) || 0,
                    - parseInt(offsetEle.style.top,0) || 0
                ];

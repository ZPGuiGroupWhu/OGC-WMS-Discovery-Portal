...
public interface LayerMapper {
    ...
    @Select("<script>" +
            ...
            "from layerlist l inner join photos p on 1=1  " +
    ...
    @Select("<script>" + 
    		"select ID as id,Abstract as abstr, ServiceID as serviceId, Attribution as attribution,Keywords as keywords,`Name` as name,Title as title,URL as url,BoundingBox as boundingbox,Topic as topic from layerlist where " + 
    		"ID in" + 
    ...
    @Select("<script>" + 
    		...
    		"la.`Name` as name, la.Title as title, la.URL as url, la.BoundingBox as boundingbox, la.Topic as topic, ph.Image as photo from layerlist la inner join photos ph on la.ID = ph.LayerID and " + 
    		"la.ID in" + 
    		"<foreach item='item' collection='layerIdArray' separator=',' open='(' close=')' index=''>" + 
    		"      #{item, jdbcType=NUMERIC}" + 
    		"    </foreach>" +

# Compare with
#   1. MDL_RM-g: no random merge, no beam search
#
...
def experience_get_specific_samples_result(part_name, sample_paths):
    ...
    finished_result_path = os.path.join(output_dir, "result_jaccard_similarity_time_use_avg" + str(run_time) + ".json")

    result = []
    finished_record_keys = {}
    if os.path.exists(finished_result_path):
        result = load_json(output_path)
        finished_result = load_json(finished_result_path)
        for x in finished_result:
            tmp_key = (x["scene"], x["feedback_noise_rate"], x["label_noise_rate"], x["method"])
            finished_record_keys[tmp_key] = x
        ...
        for tmp_feedback_noise_rate in feedback_noise_rates:
            ...
            for tmp_label_noise_rate in label_noise_rates:
                ...
                for tmp_method in methods:
                    tmp_run_num += 1
                    tmp_record_key = (tmp_sample_name, tmp_feedback_noise_rate,
                                      tmp_label_noise_rate, tmp_method)
                    if tmp_record_key in finished_record_keys:
                        result.append(finished_record_keys[tmp_record_key])
                        print(f"running: {part_name} - {tmp_run_num}/{total_run_num} - {tmp_sample_name} - "
                              f"{tmp_feedback_noise_rate} - {tmp_label_noise_rate} - {tmp_method} - skipped")
                        continue
